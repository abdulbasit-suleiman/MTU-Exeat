...
import {
  ...
  orderBy, onSnapshot, getDocs, updateDoc, serverTimestamp,
...
.frow{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px;}
...
function StudentLogin({ go, onLogin }) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      onLogin({
        role: 'student', uid: cred.user.uid, name: `${p.surname || ''} ${p.otherNames || ''}`.trim() || cred.user.email.split('@')[0],
      ...
      setErr(e.code === 'auth/invalid-credential' || e.code === 'auth/wrong-password' ? 'Incorrect email or password.' :
        e.code === 'auth/user-not-found' ? 'No account found. Please register first.' : 'Sign-in failed: ' + e.message);
    }
    setLoading(false);
  }
    ...
    <div className="auth-wrap">
      <div className="auth-left">
        <div>
          ...
          {view === 'login' ? <><h2>Student Sign In</h2><p>Sign in with your MTU credentials to access your exeat dashboard.</p></> : <><h2>Create Account</h2><p>Register once — all future logins use the email and password you set here.</p></>}
        </div>
...
function AffairsLogin({ go, onLogin }) {
  ...
  function handleLogin() { if (email === PORTAL_CREDS.affairs.email && pass === PORTAL_CREDS.affairs.password) { onLogin({ role: 'affairs', name: 'Student Affairs' }); go('affairs-dashboard'); } else { setErr('Invalid email or password.'); } }
  return (
    <div className="auth-wrap">
...
function CSOLogin({ go, onLogin }) {
  const [email, setEmail] = useState(''); const [pass, setPass] = useState(''); const [err, setErr] = useState('');
  function handleLogin() { if (email === PORTAL_CREDS.cso.email && pass === PORTAL_CREDS.cso.password) { onLogin({ role: 'cso', name: 'CSO' }); go('cso-dashboard'); } else { setErr('Invalid credentials. Access denied.'); } }
  return (
    <div className="auth-wrap">
...
/* ─── EXEAT PROGRESS TRACKER ─── */
function ExeatTracker({ exeat }) {
  const steps = [
    { key: 'submitted', label: 'Submitted' },
    { key: 'parent', label: 'Parent Consent' },
    { key: 'affairs', label: 'Affairs Review' },
    { key: 'cso', label: 'CSO Approval' },
  ];
        ...
        {pending.map(e => (
          <div key={e.id} className="exeat-card">
            ...
            <div className="ec-detail">
              <div className="ec-d-item"><label>Parent Consent</label><span>{e.parentStatus === 'approved' ? '✓ Approved' : e.parentStatus === 'declined' ? '✗ Declined' : '⏳ Awaiting'}</span></div>
              <div className="ec-d-item"><label>Affairs Review</label><span>{e.affairsStatus === 'approved' ? '✓ Cleared' : e.affairsStatus === 'declined' ? '✗ Declined' : '⏳ Awaiting'}</span></div>
              <div className="ec-d-item"><label>CSO Decision</label><span>{e.csoStatus === 'approved' ? '✓ Approved' : e.csoStatus === 'declined' ? '✗ Declined' : '⏳ Awaiting'}</span></div>
  ...
  if (tab === 'history') return (
    <>
      ...
      {modal && (
        <div className="modal-bg" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-sect">Approval Trail</div>
            {[['Parent', modal.parentStatus || 'pending'], ['Affairs', modal.affairsStatus || 'pending'], ['CSO', modal.csoStatus || 'pending']].map(([k, v]) => (
              <div className="modal-row" key={k}><span className="modal-key">{k}</span><span className="modal-val"><StatusBadge type={v} /></span></div>
            ))}
            <div className="modal-sect">Timestamps</div>
            {[['Submitted', fmtDate(modal.createdAt)], ['Parent Action', fmtDate(modal.parentActionAt)], ['Affairs Action', fmtDate(modal.affairsActionAt)], ['CSO Action', fmtDate(modal.csoActionAt)]].map(([k, v]) => v && v !== '—' ? (
              <div className="modal-row" key={k}><span className="modal-key">{k}</span><span className="modal-val">{v}</span></div>
            ) : null)}
          </div>
        </div>
      ...
      <div className="spanel">
        ...
        {loading && <div className="spinner" />}
        {!loading && history.length === 0 && <div className="empty-state"><div style={{ fontSize: 32 }}>📭</div><p>No exeat records yet. Submit your first request to get started.</p></div>}
        {!loading && (
          <table className="stable">
            <thead><tr><th>Reference</th><th>Purpose</th><th>Exit Date</th><th>Return</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {history.map(e => (
  ...
  return (
    <>
      ...
      <div className="action-grid">
        ...
        <div className="ac" onClick={() => setTab('pending')} style={{ position: 'relative' }}>
      ...
      {submitted && (
        <div className="success-banner">
          ...
          <p><strong>Exeat request submitted successfully!</strong><br />
            A consent email has been sent to {user?.parentName} at {user?.parentEmail}. You will be notified once they respond. Redirecting…</p>
        </div>
      )}
      <div className="form-card">
        ...
        {consent && user?.parentEmail && (
          <div className="consent-box">
            ...
            {user.parentName} ({user.relationship}) &nbsp;·&nbsp; {user.parentEmail} &nbsp;·&nbsp; {user.parentPhone}<br />
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-sect">Approval Trail</div>
        {[['Parent', exeat.parentStatus || 'pending'], ['Affairs', exeat.affairsStatus || 'pending'], ['CSO', exeat.csoStatus || 'pending']].map(([k, v]) => (
          <div className="modal-row" key={k}><span className="modal-key">{k}</span><span className="modal-val"><StatusBadge type={v} /></span></div>
        ))}
...
function AffairsDashboard() {
  ...
  const notifications = exeats
    ...
    .concat(exeats.filter(e => e.status === 'awaiting-cso').map(e => ({ type: 'done', title: `Forwarded to CSO — ${e.studentName}`, meta: `${e.matricNo}`, exeat: e })));

  const filtered = exeats.filter(e => {
    const s = (search || '').toLowerCase();
    const matchSearch = !s || e.studentName?.toLowerCase().includes(s) || e.matricNo?.toLowerCase().includes(s);
  ...
  async function forwardToCSO(exeat) {
    await updateDoc(doc(db, 'exeats', exeat.id), { status: 'awaiting-cso', affairsStatus: 'approved', affairsActionAt: new Date().toISOString() });
    setModal(null);
  }
  async function declineExeat(exeat) {
    await updateDoc(doc(db, 'exeats', exeat.id), { status: 'declined', affairsStatus: 'declined', affairsActionAt: new Date().toISOString() });
    setModal(null);
  }
      ...
      {tab === 'notifications' ? (
        ...
        <div className="sa-grid">
          <div className="spanel">
            ...
            <div className="sbar">
              <input type="text" placeholder="Search by name or matric number…" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} />
            ...
            {!loading && filtered.length > 0 && (
              <>
                <table className="stable">
                  <thead><tr><th>Student</th><th>Matric No.</th><th>Exit Date</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    {paged.map(e => (
                      <tr key={e.id} onClick={() => setModal(e)}>
...
function CSODashboard() {
  ...
  const total = exeats.filter(e => e.status !== 'awaiting-parent' && e.status !== 'awaiting-affairs');
  ...
  async function approveExeat(exeat) {
    await updateDoc(doc(db, 'exeats', exeat.id), { status: 'cso-approved', csoStatus: 'approved', csoActionAt: new Date().toISOString() });
    setModal(null);
  }
  async function referExeat(exeat) {
    await updateDoc(doc(db, 'exeats', exeat.id), { status: 'referred', csoStatus: 'declined', csoActionAt: new Date().toISOString() });
    setModal(null);
  }
        ...
        <div className="rec-card">
          ...
          {approved.map(r => (
            <div className="rr" key={r.id}>
              ...
              <div className="rr-r">{fmtDate(r.csoActionAt || r.createdAt)}<span className="sbadge sb-ok">✓ CSO Approved</span></div>
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  function handleLogout() { setSession(null); setPage('landing'); }
  function renderPage() {
    switch (page) {
