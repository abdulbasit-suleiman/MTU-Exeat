...
export async function POST(req) {
  try {
    const { hodEmail, studentName, matricNo, refNo, exitDate, returnDate, reason, department } = await req.json();
    if (!hodEmail) return NextResponse.json({ success: false, error: 'No HOD email provided' });
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;border:1px solid #e0d6f0;border-radius:12px;">
          <div style="background:#2B0A52;padding:20px 24px;border-radius:8px;margin-bottom:24px;">
            ...
            <p style="color:rgba(255,255,255,0.6);margin:6px 0 0;font-size:13px;">HOD Notification — Student Exeat Request</p>
          </div>
          <p style="color:#333;font-size:14px;">Dear HOD,</p>
          <p style="color:#555;font-size:13.5px;line-height:1.7;">
            A student in your department has submitted an exeat request and listed you as their HOD.
          </p>
          ...
          <p style="color:#555;font-size:13px;line-height:1.7;">Log in to the HOD Portal to view full details and track this student's exeat status.</p>
